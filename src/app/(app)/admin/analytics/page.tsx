...
import { BookOpenCheck, Bot, Download, TrendingUp, Users } from "lucide-react";
...
export default async function AdminAnalyticsPage() {
  ...
  const [weeklySessions, allAnswers, scoredSessions, examSessions, answerRows, aiUsage, installs, recentInstalls] = await Promise.all([
    ...
    admin.from("pwa_installations").select("id", { count: "exact", head: true }),
    admin.from("pwa_installations").select("id", { count: "exact", head: true }).gte("installed_at", monthStart),
  ]);
  ...
  const totalInstalls = installs.error ? null : installs.count;
  const installsThisMonth = recentInstalls.error ? null : recentInstalls.count;
  ...
  const metrics = [
    ...
    { label: "PWA installations", value: count(totalInstalls), icon: Download, note: `${count(installsThisMonth)} in the last 30 days` },
  ];
